...
def sarcasm_detection (sentence):
    ...
    feature_basictest.append(text_features.dialogue_act_features(sentence,topic_mod))
    feature_basictest=np.array(feature_basictest) 
    feature_basictestvec = vec.transform(feature_basictest)
    print (classifier.predict(feature_basictestvec))
    if (classifier.predict(feature_basictestvec) == 1):
        for word, pos in sentence:
            if pos == 'JJ':
                ...
                for syn in wn.synsets(word):
                    for l in syn.lemmas():
                        if l.antonyms():
                            sentence.replace(word,l.antonyms()[0].name())
